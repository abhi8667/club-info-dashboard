...
for seed in clubSeeds:
    ...
    events = [
        {"name": f"{name} Orientation", "date": "Aug 28", "time": "TBD", "venue": "TBD", "detail": "Introduction to our club activities and roadmap for the year."}
    ]
    ...
    if name in events_map and len(events_map[name]) > 0:
        events = []
        for e in events_map[name]:
            e_clean = e.copy()
            e_clean["time"] = "TBD"
            e_clean["venue"] = "TBD"
            events.append(e_clean)
            
    # Clean description
    desc = desc.replace('\n', ' ')
    
    clubs_json_data.append({
        "id": id_str,
        "name": name,
        "shortName": shortName,
        "category": category,
        ...
        "symbol": symbol,
        "lead": "Student Lead",
        "email": f"contact@{shortName.lower()}.rvce.edu.in",
        "instagram": f"@{shortName.lower()}_rvce",
        "venue": "Seminar Hall",
        "images": [
            f"https://picsum.photos/seed/{shortName}1/800/400",
            f"https://picsum.photos/seed/{shortName}2/800/400"
        ],
        "events": events
    })
...
with open("data/clubs.json", "w") as f:
    json.dump(clubs_json_data, f, indent=2)

# Write clubs.ts wrapper
out_ts = """import clubsData from './clubs.json'
...
print("Generated data/clubs.json and updated data/clubs.ts")
