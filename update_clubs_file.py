...
for club in scraped_clubs:
    ...
    if best_match:
        # If multiple scraped map to the same seed, extend events
        if best_match[0] in mapped_data:
            mapped_data[best_match[0]]["events"].extend(club["events"])
            if len(club["description"]) > len(mapped_data[best_match[0]]["description"]):
                mapped_data[best_match[0]]["description"] = club["description"]
        else:
            mapped_data[best_match[0]] = club

# Generate new clubs.ts
...
for seed in clubSeeds:
    ...
    if name in mapped_data:
        if mapped_data[name]["description"]:
            desc = mapped_data[name]["description"]
        if mapped_data[name]["events"]:
            events = mapped_data[name]["events"]
...
print(f"Updated clubs.ts with {len(mapped_data)} extracted clubs")
